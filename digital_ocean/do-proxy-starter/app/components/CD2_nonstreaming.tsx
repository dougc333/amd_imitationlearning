...
export default function CD2() {
  ...
  // 1. Run Setup Script (create user amd, etc.)
  const runSetupScript = async () => {
    if (!publicIPv4) return;
    setSetupSuccess(false);
    try {
      setSetupStatus("Running remote setup script…");
      ...
      if (!res.ok) {
        ...
        setSetupStatus(
          `Setup OK (exitCode=${data.exitCode ?? "?"}). stdout:\n${data.stdout ?? ""}`
        );
  ...
  // 2. Run Build vLLM Script (only available after setup succeeds)
  const runBuildScript = async () => {
    if (!publicIPv4) return;
    setBuildStatus("Starting build request...");
    try {
      setBuildStatus("Running build vllm script… (this may take a while)");
      const res = await fetch("/api/ssh/build-vllm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ip: publicIPv4 }),
      });
      ...
      if (!res.ok) {
        setBuildStatus(
          `Build failed (HTTP ${res.status}): ${
            data.error || JSON.stringify(data)
          }`
        );
      } else {
        setBuildStatus(
          `Build OK (exitCode=${data.exitCode ?? "?"}). stdout:\n${data.stdout ?? ""}`
        );
      }
    } catch (err: any) {
      setBuildStatus(`Build error: ${err?.message || String(err)}`);
    }
  };
